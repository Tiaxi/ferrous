...
- [ ] Implement test plan phase 3 (broaden backend/integration regression coverage for playback behavior). In progress: expanded process-vs-FFI parity coverage for queue transition flows, successful seek path invariants, and invalid-seek error parity in `src/bin/native_frontend.rs`; added deterministic bridge queue/play-at/seek-clamp/remove integration test and playback seek-boundary regression unit test.
