...
| Status | Commit | Pick type | Why rescue it |
| --- | --- | --- | --- |
| todo | `0f8f648` `fix: avoid centered zoom-out freeze on coarser restart` | Adapt | Prevents centered zoom-out from looking stuck on the previous fine-hop image. |
| todo | `84be591` `fix: refresh spectrogram body texture after resize` | Adapt | Ensures resize invalidates the body image generation so texture upload refreshes. |
| todo | `0d0c1c2` `fix: preserve top-bin coverage in linear spectrograms` | Direct/adapt | Fixes linear-scale bin mapping so the highest FFT bins remain visible at tall heights. |
| todo | `2cae6f5` `fix: grow rolling spectrogram cache incrementally` | Group | Avoids full rolling cache rebuilds during initial fill. |
| todo | `240d49c` `fix: smooth rolling fill-to-scroll transition` | Group | Preserves overlap when rolling fill turns into leftward motion. |
| todo | `f7ea825` `fix: trim redundant spectrogram cache clears` | Group | Removes unnecessary clearing in hot render-cache paths. |
| todo | `b7886c9` `fix: warm zoom-fill cache behind frozen frame` | Group | Lets the new cache build while the old centered frame remains visible. |
| todo | `12dc487` `fix: keep rolling prefill on a fixed-width cache` | Group | Keeps rolling prefill anchored to a full-width display cache. |
| todo | `9b092fc` `fix: segment rolling spectrogram texture uploads` | Group | Avoids repeated full texture uploads while rolling; uses base plus tail texture segments. |
| todo | `b766402` `fix: keep rolling cache incremental at fractional zoom` | Group | Preserves incremental scroll at non-integer zoom by tracking subpixel offset. |
| todo | `7a8d88b` `fix: add rolling ring headroom beyond lookahead` | Group | Adds an extra viewport of rolling ring slack so decoder lookahead does not evict live-window data. |
| todo | `45d1f7b` `fix: bound rolling spectrogram body textures` | Group | Bounds rolling body texture node/resource growth after long scrolls. |
| todo | `219707f` `fix: smooth rolling fill completion` | Group | Keeps transition from partially filled rolling viewport to full scrolling incremental. |
| todo | `b949c6b` `fix: reuse crosshair label overlay buffers` | Direct/adapt | Reduces crosshair overlay allocation churn by clearing only old/new label rectangles. |
...
| Status | Commit | Pick type | Why rescue it |
| --- | --- | --- | --- |
| todo | `f11b1ae` `fix: keep spectrogram seek targets current` | Adapt | Optimistically publishes seek target in `BridgeClient` and fixes stale centered click-to-seek mapping. |
| todo | `5a97bc7` `fix: keep centered seeks ahead of lagging decode` | Adapt | Prevents lagging decoded tail from being treated as EOF during far centered seeks. |
| todo | `a992480` `fix: clear centered spectrogram on track reset` | Adapt | Stops old centered frames from lingering on non-gapless track changes. |
| todo | `7d55997` `fix: let visual seek clock own reacquire` | Pair with `f9e07fa` | Final visual-seek-clock approach; supersedes earlier centered seek freeze experiments. |
| todo | `f9e07fa` `fix: preserve visual seek clock through target echo` | Pair with `7d55997` | Keeps the visual seek clock active when backend echoes the target or lags slightly behind it. |
| todo | `2b416d9` `fix: reset playback clock on playlist restart` | Adapt | Clears pending seek and visual clock when restarting a queue item. |
